 ...
 */
export default function HuellaIcon({ estado = 'idle' }) {
  const colorBase = {
    idle:  '#4b5563',
    scan:  '#f59e0b',
    ok:    '#10b981',
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <svg
        viewBox="0 0 120 120"
        className={`w-40 h-40 transition-all duration-500 ${estado === 'idle' ? 'animate-pulse_fp' : ''}`}
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        {/* Arcos concéntricos que forman la huella */}
        <g stroke={colorBase} strokeLinecap="round" strokeWidth="4" opacity="0.9">
          {/* Centro */}
        ...
        {estado === 'ok' && (
          <path
            ...
            className="animate-[slide_up_0.3s_ease_both]"
          />
        )}
      ...
      {estado === 'scan' && (
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div
            className="absolute left-4 right-4 h-0.5 bg-amber-400/80 blur-[1px] animate-scan"
            style={{ top: '0%' }}
          />
        </div>
      )}
